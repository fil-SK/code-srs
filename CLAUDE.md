...
The redesign proceeds in additive phases. As of the most recent phase, **Review is the first (and so far only) production surface actually wired to v2** — the live `/review` route renders through the same v2 shell `/design-preview/*` uses. Every other route/component/behavior described elsewhere in this file is still exactly as documented, unaffected.
