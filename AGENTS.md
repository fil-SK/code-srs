...
- **One storage seam.** Everything depends on `Repository` (`packages/core/src/data/repository.ts`); `getRepository()` (`src/data/index.ts`) picks `DexieRepository` or `SupabaseRepository`. **Never import a backend from a component, hook or page.**
