...
export function walkthroughFormToPreviewCard(
  ...
  id: string,
): Card & { interaction: WalkthroughInteraction } {
  return {
    id,
...
export function cardRecordToWalkthroughForm(record: Card): WalkthroughFormState {
  if (record.interaction.type !== 'walkthrough') {
    throw new Error(`Card ${record.id} is not a Walkthrough interaction`)
  }
