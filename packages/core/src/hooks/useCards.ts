...
export function useCard(id: ID | undefined) {
  return useQuery({
    ...
    // "not found" value.
    queryFn: async () => (await getRepository().cards.getById(id as ID)) ?? null,
    enabled: !!id,
  })
}
...
export function useDueCards(query: DueQuery) {
  return useQuery({
    ...
    queryFn: () => getRepository().cards.getDue(query),
  })
}
...
export function useSearchCards(query: CardQuery) {
  return useQuery({
    ...
    queryFn: () => getRepository().cards.search(query),
  })
}
...
export function useCreateCard() {
  ...
  return useMutation({
    mutationFn: async (card: Card) => {
      await getRepository().cards.put(card)
      return card
    },
...
export function useSaveCard() {
  ...
  return useMutation({
    mutationFn: async (card: Card) => {
      ...
      await getRepository().cards.put(saved)
    },
...
export function useDeleteCard() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (id: ID) => getRepository().cards.delete(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.cards }),
  })
}
...
export function useMoveCard() {
  ...
  return useMutation({
    mutationFn: async ({ card, deckId }: { card: Card; deckId: ID }) => {
      ...
      await getRepository().cards.put({ ...rest, deckId, updatedAt: Date.now() })
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.cards }),
  })
}
...
export function useReorderCards() {
  ...
  return useMutation({
    mutationFn: async (cards: Card[]) => {
      ...
      await getRepository().cards.bulkPut(ordered)
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.cards }),
  })
}
...
// domain/cards/, each unit-tested directly against a repository.
...
export function useSaveWalkthroughCard() {
  return useSaveCardMutation<WalkthroughFormState, SaveWalkthroughCardTarget>((form, target) =>
    saveWalkthroughCard(getRepository(), form, target),
  )
}
