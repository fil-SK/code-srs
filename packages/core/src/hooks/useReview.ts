...
export function useReviewLogs() {
  return useQuery({ queryKey: qk.reviewsAll, queryFn: () => getRepository().reviews.all() })
}

// What the live backend can promise for a review write, so failure copy can
// state it instead of guessing. Mirrors canReplaceConfiguredImport() in
// hooks/useBackup.ts: the UI reads one value and never branches on which
// backend is live.
export function reviewWriteGuarantee(): WriteGuarantee {
  return getRepository().reviewGuarantee
}
...
export function usePersistReviewResult() {
  ...
  return useMutation({
    mutationFn: async ({ card, after, log }: PersistReviewResultInput) => {
      ...
      await getRepository().commitReview({ card: graded, log })
    },
    onSuccess: () => {
...
export function useUndoGrade() {
  ...
  return useMutation({
    mutationFn: async ({ card, logId }: UndoInput) => {
      await getRepository().revertReview({ card, logId })
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: qk.cards })
      qc.invalidateQueries({ queryKey: qk.reviewsAll })
