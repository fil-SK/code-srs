...
export function useRoadmaps() {
  return useQuery({ queryKey: qk.roadmaps, queryFn: () => getRepository().roadmaps.getAll() })
}

export function useRoadmap(id: ID | undefined) {
  return useQuery({
    queryKey: id ? qk.roadmap(id) : qk.roadmaps,
    queryFn: () => getRepository().roadmaps.getById(id!),
    enabled: Boolean(id),
  })
}
...
export function useCreateRoadmap() {
  ...
  return useMutation({
    mutationFn: async (input: { title: string; description?: string }) => {
      ...
      await getRepository().roadmaps.put(roadmap)
      return roadmap
    },
...
export function useSaveRoadmap() {
  ...
  return useMutation({
    mutationFn: async (roadmap: Roadmap) => {
      ...
      await getRepository().roadmaps.put(saved)
      return saved
    },
    onSuccess: (saved) => {
...
export function useDeleteRoadmap() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (id: ID) => getRepository().roadmaps.delete(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.roadmaps }),
  })
}
