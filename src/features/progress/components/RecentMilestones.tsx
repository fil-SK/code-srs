import { Flame, Target, Trophy, type LucideIcon } from 'lucide-react'
...
export function RecentMilestones({ events, limit = 4 }: { events: MilestoneEvent[]; limit?: number }) {
  ...
  return (
    <div className="rounded-itera-card border border-itera-border bg-itera-surface p-5 shadow-[var(--itera-shadow-card)]">
      <h3 className="text-sm font-semibold text-itera-ink-brand">Recent milestones</h3>
      ...
      {visible.length === 0 ? (
        ...
        <ul className="mt-4 space-y-4">
          {visible.map((event) => {
            ...
            return (
              <li key={`${event.type}-${event.threshold}`} className="flex items-center gap-3">
