...
describe('computeDeckMetrics', () => {
  it('counts cards and due cards per deck, and merges in v2 card counts', () => {
    const allCards = [
    ...
    const v2CountByDeck = new Map([['d1', 2]])

    const metrics = computeDeckMetrics(allCards, dueCards, v2CountByDeck)

    expect(metricsFor(metrics, 'd1').cardCount).toBe(4) // 2 v1 + 2 v2
  ...
  it('computes masteryFraction as the share of non-suspended cards in the review state', () => {
    ...
    const metrics = computeDeckMetrics(allCards, [], new Map())
  ...
  it('returns zeroed metrics for a deck with no cards at all', () => {
    const metrics = computeDeckMetrics([], [], new Map())
