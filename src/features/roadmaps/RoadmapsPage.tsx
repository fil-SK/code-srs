...
export function RoadmapsPage() {
  ...
  function newRoadmap() {
    const title = window.prompt('Roadmap title (e.g. C++ or MLIR)')?.trim()
    if (!title) return
    create.mutate(
      { title },
  ...
  function deleteRoadmap(r: Roadmap) {
    if (window.confirm(`Delete the roadmap “${r.title}”? Decks are not affected.`))
      remove.mutate(r.id)
  }
