import { useState } from 'react'
import { Button } from '@/components/ui/Button'
import { initialSchedulingState } from '@/domain/scheduling/state'
import {
  matchingFormToPreviewCard,
  type MatchingFormState,
} from '@/domain/cardsV2/matchingForm'
import { getInteractionDefinition } from '@/features/reviewV2/interactions/registry'
import { ReviewSessionScreen } from '@/features/reviewV2/ReviewSessionScreen'
import { IteraSurface } from '@/features/reviewV2/components/IteraSurface'

// The editor's live preview: the *actual* production Matching interaction
// component, not a fake mockup — same ReviewSessionScreen + registry Review
// uses. Non-committing: schedulingBefore is always a fresh baseline and
// onExit is a no-op.
//
// Like OrderingLivePreview/MultipleChoiceLivePreview, ReviewSessionScreen
// only resets its internal phase/response state on remount, so this keys on
// an explicit resetKey counter, not on form content: normal edits (typing,
// adding a row) update the live card without discarding an in-progress
// pairing, and "Reset preview" is the only thing that remounts back to the
// unanswered state.
export function MatchingLivePreview({ form }: { form: MatchingFormState }) {
  const [resetKey, setResetKey] = useState(0)
  const card = matchingFormToPreviewCard(form, 'preview')

  return (
    <IteraSurface>
      <div className="mb-3 flex justify-end">
        <Button type="button" variant="ghost" onClick={() => setResetKey((k) => k + 1)}>
          Reset preview
        </Button>
      </div>
      <ReviewSessionScreen
        key={resetKey}
        card={card}
        definition={getInteractionDefinition('matching')}
        current={1}
        total={1}
        onExit={() => {}}
        schedulingBefore={initialSchedulingState()}
      />
    </IteraSurface>
  )
}
