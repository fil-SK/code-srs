...
export function multipleChoiceFormToPreviewCard(
  ...
  id: string,
): Card & { interaction: MultipleChoiceInteraction } {
  return {
    id,
...
export function cardRecordToMultipleChoiceForm(record: Card): MultipleChoiceFormState {
  if (record.interaction.type !== 'multiple_choice') {
    throw new Error(`Card ${record.id} is not a Multiple Choice interaction`)
  }
  return {
