...
describe('cardRecordToWalkthroughForm round-trip', () => {
  const record: Card = {
    id: 'card-1',
    schemaVersion: 2,
    deckId: 'deck-1',
  ...
  it('round-trips a Card back into form state, including multi-range focus', () => {
    const form = cardRecordToWalkthroughForm(record)
  ...
  it('formToRecord(recordToForm(record)) reproduces the same interaction shape', () => {
    const roundTripped = walkthroughFormToRecord(
      cardRecordToWalkthroughForm(record),
      {
        id: record.id,
  ...
  it('throws for a Card that is not Walkthrough-shaped', () => {
    const recallRecord: Card = {
      ...record,
      interaction: { type: 'recall', answer: { format: 'markdown', value: 'A' } },
    }
    expect(() => cardRecordToWalkthroughForm(recallRecord)).toThrow()
  })
})
