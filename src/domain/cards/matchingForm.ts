...
// both unreadable and too narrow to hold real text at the card's width.
...
export function matchingFormToPreviewCard(
  ...
  id: string,
): Card & { interaction: MatchingInteraction } {
  return {
    id,
...
export function cardRecordToMatchingForm(record: Card): MatchingFormState {
  if (record.interaction.type !== 'matching') {
    throw new Error(`Card ${record.id} is not a Matching interaction`)
  }
