...
describe('cardV2RecordToMultipleChoiceForm / legacyMcqCardToForm round-trips', () => {
  ...
  it('round-trips a CardV2Record back into form state', () => {
    expect(cardV2RecordToMultipleChoiceForm(record)).toEqual({
      deckId: 'deck-1',
      prompt: 'Q',
      tip: 'hint',
      explanation: 'why',
  ...
  it('formToRecord(recordToForm(record)) reproduces the same interaction shape', () => {
    const roundTripped = multipleChoiceFormToRecord(
      cardV2RecordToMultipleChoiceForm(record),
  ...
  it('throws for a CardV2Record that is not Multiple Choice-shaped', () => {
    const recallRecord: CardV2Record = {
      ...record,
      interaction: { type: 'recall', answer: { format: 'markdown', value: 'A' } },
    }
    expect(() => cardV2RecordToMultipleChoiceForm(recallRecord)).toThrow()
  })

  it('hydrates from a legacy v1 mcq card via migrateCard', () => {
    const card: Card = {
      id: 'c1',
      deckId: 'd1',
      tags: ['tag'],
    ...
    expect(legacyMcqCardToForm(card)).toEqual({
      deckId: 'd1',
      prompt: 'Which is true?',
      tip: '',
      explanation: 'because',
