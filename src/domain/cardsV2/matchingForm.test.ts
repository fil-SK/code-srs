...
describe('matchingFormToPreviewCard - two column', () => {
  it('builds a CardV2 with source + one other column, ids derived from rows/columns', () => {
...
describe('cardV2RecordToMatchingForm round-trip', () => {
  ...
  it('round-trips a CardV2Record back into form state', () => {
    const form = cardV2RecordToMatchingForm(record)
  ...
  it('formToRecord(recordToForm(record)) reproduces the same interaction shape', () => {
    const roundTripped = matchingFormToRecord(
      cardV2RecordToMatchingForm(record),
      {
        id: record.id,
  ...
  it('throws for a CardV2Record that is not Matching-shaped', () => {
    const recallRecord: CardV2Record = {
      ...record,
      interaction: { type: 'recall', answer: { format: 'markdown', value: 'A' } },
    }
    expect(() => cardV2RecordToMatchingForm(recallRecord)).toThrow()
  })
})

describe('legacyMatchingCardToForm', () => {
  it('hydrates a two-column legacy card and preserves relationships losslessly through a full round-trip', () => {
    const card: Card = {
      id: 'c1',
      deckId: 'd1',
      tags: ['tag'],
      ...
      content: {
        prompt: 'Match',
        pairs: [
          { id: 'p1', left: 'Stack', right: 'LIFO' },
          { id: 'p2', left: 'Heap', right: 'Manual' },
        ],
      },
    }
    const hydrated = legacyMatchingCardToForm(card)
    expect(hydrated.deckId).toBe('d1')
    expect(hydrated.rows.map((r) => r.source)).toEqual(['Stack', 'Heap'])
    expect(hydrated.tags).toBe('tag')

    const record = matchingFormToRecord(hydrated, {
      id: 'c1',
      createdAt: 1,
      suspended: false,
      scheduling,
    })
    // Same shape migrateCard would have produced directly (ids re-derived by
    // this file's own uniqueItemId convention match migrateMatching's).
    const migratedDirectly = matchingFormToRecord(legacyMatchingCardToForm(card), {
      id: 'c1',
      createdAt: 1,
      suspended: false,
      scheduling,
    })
    expect(record.interaction).toEqual(migratedDirectly.interaction)
  })

  it('hydrates a three-part, fixed-option legacy card', () => {
    const card: Card = {
      id: 'c2',
      deckId: 'd1',
      tags: [],
      ...
      content: {
        ...
        triple: true,
        pairs: [
          { id: 'p1', left: 'A', right: 'yes', third: 'x' },
          { id: 'p2', left: 'B', right: 'no', third: 'x' },
        ],
        options: { right: ['yes', 'no'] },
      },
    }
    const hydrated = legacyMatchingCardToForm(card)
    expect(hydrated.columns).toHaveLength(2)
    const fixedCol = hydrated.columns.find((c) => c.fixed)
    expect(fixedCol?.options.map((o) => o.text)).toEqual(['yes', 'no'])
    const uniqueCol = hydrated.columns.find((c) => !c.fixed)
    expect(uniqueCol?.id).toBe('third')
    ...
    expect(record.interaction.relationships).toHaveLength(2)
  })

  it('throws for a legacy card that is not Matching-shaped', () => {
    const card: Card = {
      id: 'c3',
      deckId: 'd1',
      tags: [],
    ...
    expect(() => legacyMatchingCardToForm(card)).toThrow()
  })
})
