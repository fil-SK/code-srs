...
// The Write Code editor's single save path — mirrors saveMultipleChoiceCard.ts
// exactly: a brand-new card, editing an existing CardV2Record, or editing (and
// thereby migrating) a legacy v1 codeCompletion card. The v1 case reuses the
// original card's id/createdAt/scheduling/suspended, then deletes the
// superseded v1 Card row and its cardStates dual-write mirror — one persisted
// record per card.
