...
describe('cardV2RecordToForm / legacyCardToForm round-trips', () => {
  it('round-trips a CardV2Record back into form state', () => {
    ...
    expect(cardV2RecordToForm(record)).toEqual({
      deckId: 'deck-1',
      prompt: 'Q',
      tip: 'hint',
      explanation: 'why',
  ...
  it('hydrates from a legacy v1 basic card via migrateCard', () => {
    const card: Card = {
      id: 'c1',
      deckId: 'd1',
      tags: ['tag'],
    ...
    expect(legacyCardToForm(card)).toEqual({
      deckId: 'd1',
      prompt: 'Q',
      tip: '',
      explanation: 'why',
