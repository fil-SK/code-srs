...
import {
  ...
  type WalkthroughEnvelope,
  type WalkthroughFormState,
} from './walkthroughForm'

export type SaveWalkthroughCardTarget =
  | { kind: 'new' }
  | { kind: 'v2'; record: CardV2Record }
  | { kind: 'v1'; card: Card }

// The Walkthrough editor's single save path — mirrors saveMatchingCard.ts
// exactly: a brand-new card, editing an existing CardV2Record, or editing
// (and thereby migrating) a legacy v1 story card. The v1 case reuses the
...
export async function saveWalkthroughCard(
  ...
  const record = walkthroughFormToRecord(form, envelope, now)
