...
describe('writeCodeFormToPreviewCard', () => {
  it('builds a CardV2 with the given id and no timestamps', () => {
...
describe('cardV2RecordToWriteCodeForm / legacyWriteCodeCardToForm round-trips', () => {
  ...
  it('round-trips a CardV2Record back into form state', () => {
    const f = cardV2RecordToWriteCodeForm(record)
  ...
  it('throws for a CardV2Record that is not Write-Code-shaped', () => {
    const recallRecord: CardV2Record = {
      ...record,
      interaction: { type: 'recall', answer: { format: 'markdown', value: 'A' } },
    }
    expect(() => cardV2RecordToWriteCodeForm(recallRecord)).toThrow()
  })

  it('hydrates from a legacy v1 codeCompletion card via migrateCard', () => {
    const card: Card = {
      id: 'c1',
      deckId: 'd1',
      tags: ['tag'],
      ...
      content: {
        prompt: 'Complete it.',
        scaffold: { language: 'rust', code: 'fn total(v: &[i32]) -> i32 {\n}' },
        solutions: ['fn total(v: &[i32]) -> i32 {\n    v.iter().sum()\n}'],
        validation: { mode: 'normalizedMatch', ignoreWhitespace: true, caseSensitive: false },
        explanation: 'Iterator::sum does the work.',
      },
    }
    const f = legacyWriteCodeCardToForm(card)
    expect(f.deckId).toBe('d1')
    expect(f.prompt).toBe('Complete it.')
    expect(f.explanation).toBe('Iterator::sum does the work.')
    expect(f.language).toBe('rust')
    expect(f.starterCode).toBe('fn total(v: &[i32]) -> i32 {\n}')
    expect(f.acceptedAnswers.map((a) => a.code)).toEqual([
      'fn total(v: &[i32]) -> i32 {\n    v.iter().sum()\n}',
    ])
    expect(f.comparison).toEqual({
      trimOuterWhitespace: true,
    ...
    expect(f.tags).toBe('tag')
  })

  it('throws for a legacy card that is not Write-Code-shaped', () => {
    const card: Card = {
      id: 'c2',
      deckId: 'd1',
      tags: [],
    ...
    expect(() => legacyWriteCodeCardToForm(card)).toThrow()
  })
})
