...
| `/design-preview/library`, `/design-preview/library/:deckId` | `library-browser/`, `library-deck/`, `library-shared/` (all under `design-preview/`) | Library browser (Collection nav + Deck list) and focused Deck page (Collection nav + Cards/Insights tabs) against hand-authored fixtures — Phase H's visual direction, checked ahead of the real Collection/Deck migration. | **Itera redesign, placeholder/preview.** No real `Collection` type, no data migration, no Create/Edit — invented local fixture types only. Not linked from any real nav. See `itera-redesign-plan.md` Phase H and `itera-decisions.md` D55-D58. |
