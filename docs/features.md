...
| `/preview` | `PreviewPage` | Flip through a deck's cards (or one card, from Browse) with no scheduling impact: the shared Review strip (**< Exit session**, bold position, keycap shortcut), prev/next, and arrow keys. The card itself is the real Review experience — `migrateCard` + `ReviewSessionScreen` — the same path `/review` takes. | **Itera redesign, production.** The old tag/jump-input strip is gone. `AppShell` gives this route a full-width, zero-top-padding main: the white strip spans the page beneath the navbar, but its controls align to the navbar's centered logo/profile edges. Revealed cards say **Answer revealed** because this route hides rating controls; the final content retains the shared symmetric 56px bottom frame. The remaining prev/next page controls still wrap the shared Review surface. |
