...
| Editor / authoring | yes (`Editor` component, `emptyContent`) | Recall, Multiple Choice, and Write Code only, as of Phase F (`src/features/cardsV2/`) — see "Card creation" below. The other three types have no editor yet; authoring for them still happens against v1 `Card`s. |
...
### Card creation (Phase F, Recall + Multiple Choice + Write Code)

`src/features/cardsV2/` is where the v2 authoring UI lives, built against `CardV2Record` (`src/types/cardV2.ts`) — `CardV2` plus its own embedded `suspended`/`scheduling`/`order`, the same envelope shape v1 `CardBase` uses, backed by the real `cardsV2` repository member above. This is a deliberate, separate store from `CardState` (the Phase D dual-write target) — `CardV2Record`'s scheduling is never read from or written to `cardStates`, so authoring a Recall, Multiple Choice, or Write Code card doesn't touch or advance that migration.

- `CardTypeChooser.tsx` — all six interaction tiles; Recall, Multiple Choice, and Write Code are enabled (`ENABLED` array), the rest render muted with a "Coming soon" caption.
- `RecallEditorShell.tsx` / `MultipleChoiceEditorShell.tsx` / `WriteCodeEditorShell.tsx` — one editor/preview shell per type (structurally identical, not a shared component — see `itera-decisions.md` D71/D74 for why extraction keeps being deferred): `RecallFields.tsx` (prompt/answer/tip/explanation/preset), `MultipleChoiceFields.tsx` (prompt/tip/explanation/selection-mode/randomize/option list via `MultipleChoiceOptionRow.tsx`), or `WriteCodeFields.tsx` (prompt/language/starter code via `LazyCodeEditor`/accepted-answer list via `WriteCodeAnswerRow.tsx`, each row its own `LazyCodeEditor`/comparison toggles kept inside a collapsed `<details>`) + Deck/Tags on the left, the matching `*LivePreview.tsx` on the right — which render the *actual* `ReviewSessionScreen`/interaction definition, not a mockup. Recall's preview is keyed on the authoring preset so typing doesn't reset an in-progress flip; Multiple Choice's and Write Code's previews instead have an explicit "Reset preview" button (a local remount-key counter) since they have no preset to key on and a submitted response only clears on remount. Desktop: two-pane grid. Below `useIsWideEditor`'s ~980px breakpoint: Editor/Preview tabs.
- `src/domain/cardsV2/recallForm.ts` / `multipleChoiceForm.ts` / `writeCodeForm.ts` — pure conversions between each editor's form state and: a throwaway preview `CardV2`, a persisted `CardV2Record`, and (via `legacyCardToForm`/`legacyMcqCardToForm`/`legacyWriteCodeCardToForm`, wrapping `migrateCard`) a legacy v1 card being hydrated for edit. `multipleChoiceForm.ts` additionally exports `validateMultipleChoiceForm` (≥2 options, all filled, ≥1 correct, single-select forbids >1 correct — enforced by trimming on the multiple→single transition, not a blocking error); `writeCodeForm.ts` exports `validateWriteCodeForm` (prompt required, ≥1 accepted answer, every accepted answer non-blank — starter code is deliberately optional).
- `src/domain/cardsV2/saveRecallCard.ts` / `saveMultipleChoiceCard.ts` / `saveWriteCodeCard.ts` — the single save path per type (unit-tested directly against a repository, independent of the `useSaveRecallCard`/`useSaveMultipleChoiceCard`/`useSaveWriteCodeCard` hooks that wrap them), each covering three targets: `new` (fresh `CardV2Record`), `v2` (update in place, same id/scheduling), and `v1` (the **legacy cutover** — builds a `CardV2Record` reusing the original v1 card's `id`/`createdAt`/`scheduling`/`suspended` so `ReviewLog` history keeps resolving, writes it to `cardsV2`, then deletes the superseded `cards` row and its `cardStates` mirror).
- `CardEditEntry.tsx` (the element at `cards/:id/edit`) — branches: a `CardV2Record` opens its matching editor by `interaction.type`; a legacy v1 `basic`/`codeReading`/`bugFinding` card opens `RecallEditorShell`, a legacy v1 `mcq` card opens `MultipleChoiceEditorShell`, and a legacy v1 `codeCompletion` card opens `WriteCodeEditorShell`; any other v1 type falls through to the untouched `CardEditorPage`. Legacy cards only migrate to `CardV2Record` when actually edited-and-saved this way — not in bulk.
