...
6. **Card content is data, never markup — and parsing is shared while rendering is not.** A card's text arrives from the learner *or* from an imported backup, is persisted, and is shown to whoever opens the card later, so it is untrusted by construction. `packages/core/src/content/` turns it into a closed union of semantic nodes carrying only plain strings; each platform renders that tree with its own elements. No production module in `apps/web/src/` may convert a string into markup or code — `dangerouslySetInnerHTML`, `innerHTML`, `outerHTML`, `insertAdjacentHTML`, `document.write`, `DOMParser`, `eval(`, `new Function(` — and `apps/web/src/components/text/renderingSinks.test.ts` fails the build if one appears. The native renderer (`apps/mobile/src/components/text/RichTextNative.tsx`) has the equivalent boundary, enumerated for its own platform rather than assumed safe because React Native has no DOM: no `Linking`, no `WebView`, no style or module name derived from card text, and the one URL sink in the card model (`WalkthroughInteraction.image`) gated by core's shared `isSafeImageSource`. `apps/mobile/src/components/text/richTextSecurity.test.tsx` drives the same `attackPayloads.ts` fixture through it and scans the component tree for those sinks. The corollary matters as much: because rendering is safe, content is **never sanitized**. Angle brackets, generics, templates and whole HTML examples are legitimate flashcard material and survive byte for byte through import, storage and display.
...
vitest.config.ts      the one runner; declares both the `web` and `core` projects
.oxlintrc.json        one lint config for the whole tree
vercel.json           SPA rewrite + outputDirectory: apps/web/dist
.env.local.example    environment stays repo-root-owned (see below)
docs/  supabase/      repository-level, not app-level

apps/web/             @itera/web - the web application (map below)
  package.json          every web dependency; supplies core's react +
                        @tanstack/react-query peers
  index.html  public/   the Vite root's entry and static assets
  vite.config.ts        plugins, PWA, '@' -> ./src, envDir -> the repo root
  tsconfig.app.json     the app project      (extends ../../tsconfig.base.json)
...
**One consequence worth knowing:** Tailwind v4 auto-detects its source files from the Vite root, so it now scans `apps/web` rather than the whole repository. That dropped 20 bare utility classes from the compiled stylesheet (68.43 kB -> 66.79 kB) which existed only because Tailwind had been reading class names out of `docs/*.md` and the agent instruction files. Every one was verified unused by the app; no variant the app actually renders (`hover:underline`, `disabled:opacity-50`, `focus-visible:ring-offset-2`, …) was affected.

**`@itera/core` is the one shared package.** It is consumed as **TypeScript source**: its `main`/`types` both point at `packages/core/src/index.ts`, there is no build step, no `dist/`, no emitted declarations and no watch process. Resolution is the ordinary npm workspace symlink (`node_modules/@itera/core` -> `packages/core`), which is why TypeScript needs no `paths` entry for it and why **neither `apps/web/vite.config.ts` nor `vitest.config.ts` contains an alias for it**. One mechanism, not three; see `itera-decisions.md` D282/D283.
...
└── apps/web/src/
    ...
    │                  values a platform with no CSS can read. apps/web/src/index.css
    ...
    │                  be apps/web/src/domain/, moved verbatim
...
- **`newId()` cannot name the `crypto` global.** It declares the two members it uses (`randomUUID?`, `getRandomValues`) structurally and reads them off `globalThis` **at call time**. Runtime behaviour is unchanged: native `randomUUID()` when available, the secure `getRandomValues` v4 fallback otherwise. Expo Go supplies neither global itself, so `apps/mobile/index.ts` installs SDK 54's `expo-crypto` bridge before importing `expo-router/entry`; core imports no platform package or polyfill.
- **`URLSearchParams` is not available either**, which is why `selectionToSearchParams`/`selectionFromSearchParams` stayed in the web `apps/web/src/features/library/collectionTree.ts` while every Collection *derivation* function moved. The `LibrarySelection` type is core's, so there is still one definition of what a selection is.

**Core and the web app are separate Vitest projects** (`vitest.config.ts`), and the top-level `npx vitest run` runs both. The web project owns `apps/web/src/**/*.test.{ts,tsx}` with the `@` alias, the blanked `VITE_SUPABASE_*` env and `apps/web/src/test/setup.ts` (fake-indexeddb plus an explicit `configureRepository(() => new DexieRepository())`). The core project owns `packages/core/src/**/*.test.ts` with **no setup file at all** — no fake IndexedDB, no alias, no configured backend. That separation exists because the alternative silently undermines the package boundary: running core's tests through the web bootstrap would prove core only under a browser storage shim with a backend it is not allowed to know about already registered, and a core test that came to depend on either would pass here and fail under Metro. A core test that needs any of it is telling you something about the code it covers.

Each project's `root` points at the package it covers (`apps/web`, `packages/core`), but the config itself stays at the repository root so `npx vitest run` remains the one command that runs both. A consequence: **a web test that reads source off disk must resolve from `import.meta.url`, never from `process.cwd()`**, because the runner is invoked from the repository root while the files it scans live under `apps/web`. The three that do this — `design/tokenDrift.test.ts`, `components/text/renderingSinks.test.ts` and `auth/storageIsolation.test.ts` — are guards whose failure mode when mis-pointed is to scan nothing and pass, which is worse than not existing.

**Core's tests are typechecked**, unlike the web app's. `tsconfig.core.test.json` is a second project over the same directory that adds `types: ["node"]` (still no DOM) so a test may use `process.env.TZ` or `globalThis.crypto`, while `tsconfig.core.json` excludes `*.test.ts` and `apps/web/src/test/` and keeps `types: []`. Never merge the two: the empty `types` is the enforcement. `tsc -b` runs both, so core source is checked with nothing ambient *and* with Node. Neither project maps the `@/` alias, so **core's tests import by relative path** — a moved test that keeps an `@/domain/...` specifier runs under Vitest and fails the typecheck.

Two support modules deliberately sit in core *source* rather than under `apps/web/src/test/`: `domain/io/backupFixtures.ts` and `data/supabase/fakeSupabaseClient.ts`. `platformNeutrality.test.ts` skips `*.test.ts` and the whole `apps/web/src/test/` directory, and so does `tsconfig.core.json`, so parking a 380-line fake server there would quietly exempt it from every guard the shipped code answers to. Neither is exported from the barrel. The one consumer outside the package is `apps/web/src/data/supabase/backendParity.test.ts`, which imports the fake by source path (`@itera/core/src/data/supabase/fakeSupabaseClient`) — the single, test-only exception to the one-entry-point rule, taken because the alternative was putting a test double on the production surface.

`apps/web/src/types/*` remains as a **transitional re-export shim** so that relocating the contracts did not have to be the same commit as rewriting ~145 files' imports, and the domain move added the same kind of shim at `apps/web/src/domain/*`, `apps/web/src/lib/{id,shuffle}.ts` and `apps/web/src/data/repository.ts`, the hook move at `apps/web/src/hooks/*` and `apps/web/src/data/index.ts`. They define nothing; `apps/web/src/types/coreSurface.test.ts` asserts at runtime that the shim and the package are the same module instance - by **reference equality**, extended past `richText` to `computeStreak`, `reviewState`, `computeRetention`, `localDayIndex`, `gradeMatching`, `parseBackup`, `newId` and `leafDecks` - so a redefinition cannot pass review unnoticed (D285/D286). **New code should import from `@itera/core` directly.**
...
**The platform composes; core does not.** The web app's single composition point is `apps/web/src/main.tsx`, before `createRoot`:
...
chosen purely by whether `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` are set (`isSupabaseConfigured` in `apps/web/src/data/supabase/client.ts`) — the **same** value `AuthProvider` reads, so backend mode and authentication mode cannot disagree (audit P1-3). **This is where configuration is read and where the browser Supabase client is built**, and the only place either happens: `SupabaseRepository` lives in `@itera/core` and takes a ready client, so the same class serves a future Expo app configured from `EXPO_PUBLIC_*` at its own entry point. `apps/web/src/data/supabase/client.ts` keeps the env lookup, `isSupabaseConfigured`, `createClient` and the browser session options (`persistSession`, `autoRefreshToken`, `detectSessionInUrl`) and must not move into core. `apps/web/src/data/index.ts` is now a re-export shim over the registry and holds no selection logic.

The web test suite configures Dexie explicitly in `apps/web/src/test/setup.ts`. It used to get Dexie *by omission* — the `VITE_SUPABASE_*` vars are blanked and the old `getRepository()` fell back to local storage — and that implicit path is gone with the default.

**Dexie backend** (`apps/web/src/data/dexie/db.ts` + `DexieRepository.ts`) — the database is named `'itera'`; it was renamed from `'code-srs'` when the card models converged, and `db.ts` fire-and-forget `Dexie.delete('code-srs')`s the superseded prototype database to reclaim its storage. (The *backup file's* `app` marker is a separate thing and deliberately still reads `code-srs` — see "Backup" below.) Schema is additive/incremental:
...
| **Roadmap** | `packages/core/src/types/roadmap.ts` | `CrudRepo<Roadmap>` | `roadmaps` | `roadmaps` | hand-rolled SVG canvas in `apps/web/src/features/roadmaps/` (no graph library) |
...
4. `apps/web/src/features/reviewV2/interactions/<type>/` — a `View` component plus an `index.ts` exporting `{ ...<type>Behavior, View }` as a `WebInteractionDefinition`. The binding spreads the shared behavior; it never restates a field.
5. `apps/web/src/features/reviewV2/interactions/registry.ts` — register it. **The registry is `Partial<...>`, so a missing entry throws at runtime rather than failing the build** — this is the one step the compiler does not enforce. When a native app exists it repeats steps 4-5 with its own View and its own registry over the *same* behavior object.
...
9. `apps/web/src/features/cards/` — `<Type>EditorShell`, `<Type>Fields`, `<Type>LivePreview`, a `CardTypeChooser` tile, and arms in `CardCreatePage` + `CardEditEntry`.
10. `apps/web/src/features/cards/shared/interactionTypeMeta.ts` — label, icon and tile colour (`features/library/shared/rowVisuals.ts` reads this for table rows).

---

## Routing

`apps/web/src/app/router.tsx` uses `createBrowserRouter` with **three structurally separate top-level entries** (not one nested tree) — since the App Shell convergence milestone (2026-07-27), Today and Review have swapped places in this list relative to earlier docs:
...
Web-specific, in `apps/web/src/auth/`:
...
`IteraSurface`/`ForceLightTheme` (`apps/web/src/features/reviewV2/components/`) are the shared root used by `AppShell`, `PreviewShell` (design-preview), `ReviewPage`/`ReviewSessionV2` and `RouteError` — one mechanism, not separate "real" vs. "preview" copies. See [`docs/design-system.md`](design-system.md) for what they do and why. See `docs/itera-decisions.md`'s "App Shell and Visual Foundation Convergence" entry for the full rationale.
...
**`apps/web/src/features/review/useSessionQueue.ts` freezes the queue when a session starts.** Everything `ReviewPage` renders below the fetch reads that snapshot, never the live query.
...
- **`deckMetrics.ts`** — moved here from `apps/web/src/features/library/`. Today's Continue Learning and Next Milestone need the same per-deck due/last-studied/mastery numbers the Library shows, and `apps/web/src/domain` may not import from `apps/web/src/features`.
...
### `apps/web/src/features/reviewV2/` — the v2 Review shell
...
`apps/web/src/features/reviewV2/interactions/types.ts` owns the web half:
...
and each `interactions/<type>/index.ts` is one line of composition — `{ ...matchingBehavior, View: MatchingView }`. **No web module reimplements `interactive`, `isResponseReady`, `autoGrade` or `widthFor`**; `apps/web/src/features/reviewV2/interactions/registry.test.ts` asserts each registered definition carries core's functions *by reference*, which is the only check that catches a copy.
...
**That prediction has since been tested.** `apps/mobile/src/components/review/` now carries the native half: `interactions/types.ts` declares `NativeInteractionDefinition` and `NativeInteractionViewProps` (which add `setScrollEnabled`, a gesture-arbitration concern web does not have), `interactions/registry.ts` is the same one-line-per-type `Partial` binding with its own by-reference test, and `reviewPhase.ts` is a second, native phase machine with four phases rather than five — web's `persistFailed` exists for a `Repository` write that can reject, and the demo session's in-memory write cannot. Nothing in `apps/mobile/src/` reimplements `interactive`, `isResponseReady`, `autoGrade` or `widthFor`. The split held: what differs between the two platforms is chrome, input and gesture handling; what decides an outcome is shared. The native shell also serves card study, through a discriminated `mode: 'review' | 'study'` on its props rather than a second screen: study uses the same phase machine, the same registry binding and the same four shared behavior members, and simply has no `schedulingBefore`, no `onGraded` and no rating control to reach for - which is why it can record nothing.
...
`apps/web/src/features/cards/` is where the authoring UI lives, built against the one `Card` type and backed by the `cards` repository member above.
...
`vitest.config.ts` declares **two projects**, not one global config (see "Core and the web app are separate Vitest projects" above). Both set `environment: 'node'` (no DOM, fast); the **web** project additionally **blanks `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`** and loads `apps/web/src/test/setup.ts`, so every web test hits the local Dexie backend via `fake-indexeddb` regardless of a developer's real `.env.local`, while the **core** project has no setup file, no alias and no env at all. `globals` is not enabled in either, so every test file imports `describe`/`it`/`expect` explicitly from `vitest`.
...
The earlier entries here are **resolved, not open**: the v1 `ReviewSession`/`useReviewSession` and `apps/web/src/components/ui/FlipCard.tsx` were deleted on 2026-08-17, and the **two card models** converged into one on 2026-08-18. There is one Review surface, one flip primitive, and one card model.
