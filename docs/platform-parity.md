...
| Capability | Shared logic (packages/core) | Web | Native | Status / reason |
|---|---|---|---|---|
| Collection tree | library/collectionTree | yes | yes | implemented (demo/local) — derived from canonical Deck.parentId on both platforms, through the same helpers; the parallel mobile collection model is deleted; both platforms also list the cards filed on a collection itself, which is what a promoted deck leaves behind; D422, D445 |
...
| Deck create / rename / delete | hooks/useDecks, domain/decks/deckForm, domain/decks/deletion | yes | yes (demo) | implemented (demo/local) — New Deck on All Decks and inside a Collection, **New deck inside** on a deck (which is how a collection is made, since a collection IS a deck with children), Edit deck (name + description) and Delete deck behind the shared `checkDeckDeletion` guard, all through the shared hooks; parent selection (Move Deck) stays web-only for now; D427, D428, D429, D444 |
