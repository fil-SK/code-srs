...
**Files (as actually shipped, superseding the list below):** `src/features/library/{LibraryBrowserPage,LibraryDeckPage,DeckRow,DeckSettings,FilterMenu,collectionTree,deckMetrics,deckMark}.tsx|ts`, `src/features/library/shared/{CollectionNav,CollectionNavDrawer,DeckMark,MasteryRing,MeterBar,EmptyState,useIsWideLibrary}.tsx|ts` (adapted from `design-preview/library-shared/*`, not imported from it — design-preview stays untouched/isolated), `src/components/layout/{TopNav,primaryNavLinks,CreateMenu,ProfileMenu}.tsx`.
