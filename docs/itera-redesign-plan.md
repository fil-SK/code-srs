...
5. Six interaction renderers, in this order: Recall (already built in Phase B — reused, not rebuilt), Multiple Choice (from `mcq`), Write Code (from `codeCompletion`), Ordering (existing, restyle + verify keyboard reordering — see the accessibility criteria below, this is not optional polish), Matching (existing, restyle, preserving 3-part/fixed-column capability per the locked decision in `itera-decisions.md`), Walkthrough (from `story`; reuse `highlightLines`/`lineRanges.ts` directly, preserving multi-range highlighting per the same locked decision).
