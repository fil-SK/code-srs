...
**Correction (2026-07-27):** the "zero navigation chrome visible inside the session" acceptance criterion below was **not actually met** until the App Shell convergence milestone (see `itera-decisions.md` D90) — `/review` was, until then, a plain child of `AppShell`, rendering inside the full sidebar/topbar on desktop like every other route, not chrome-free. It is now a genuinely separate top-level route with no `AppShell` ancestor at all.
...
**Status (2026-07-27): the global shell rewrite and the real (non-preview) Library/Deck UI have shipped — but Phase G (below) has still never run, so this ships against a UI-only Collection derivation, not a migrated `Collection` entity.** See `itera-decisions.md`'s "App Shell and Visual Foundation Convergence" entry (D88-D92) for full detail; summary:

- **Global shell rewrite (done):** `AppShell.tsx` now renders `TopNav` (`Today · Library · Progress`, Search, `+Create`, Profile) instead of `Sidebar`/`BottomNav`, replacing the entire left-sidebar/per-route-topbar frame every production route (except Review, see Phase E's now-accurate status) renders inside. `Sidebar.tsx`/`BottomNav.tsx`/`navItems.ts`/`PageHeaderOverride.tsx`/`TodayShell.tsx` were deleted (chrome code, no persisted state). New: `src/components/layout/{TopNav,primaryNavLinks,CreateMenu,ProfileMenu}.tsx`.
- **Library/Deck UI (done, promoted from the `/design-preview/library*` preview below to production):** `src/features/library/{LibraryBrowserPage,LibraryDeckPage}.tsx` (not `LibraryPage.tsx`/`DeckPage.tsx` as originally named) now render at `/decks`/`/decks/:id`, replacing `DecksPage.tsx`/`DeckDetailPage.tsx` — same paths, no route rename, since existing deep links (`/review?deck=`, `/preview?deck=`, every "back to deck" redirect) already hardcode `/decks/:id`. `DecksPage.tsx`/`DeckDetailPage.tsx` are unrouted but not deleted, pending independent parity re-confirmation.
- **Collection derivation (deliberately narrower than this phase's original spec):** `src/domain/collections/tree.ts` operating on a real, migrated `Collection` type was **not** built. Instead `src/features/library/collectionTree.ts` derives "Collection" purely structurally from the existing `Deck.parentId` tree (any deck with children is a Collection node; childless decks are the browsable Library decks) — because this milestone's instructions forbade any schema migration, and Phase G's preflight report has still never run. This is documented as D88, a real scope narrowing, not an oversight.
- **Roadmaps:** hidden from `primaryNavLinks.ts` (per the already-settled D11/D17), route/data fully preserved, reachable only via direct URL for now — exactly the "hidden starting here" this phase's original text called for, just via the real shell now shipped.
- **Collection identity view (done, 2026-07-28):** opening a Collection (a Deck-with-children) now renders `LibraryCollectionView.tsx` — a header/toolbar/list/footer composition sharing the focused Deck page's shell primitives — instead of falling back to the plain "browse everything" `LibraryBrowserPage` composition. See `itera-decisions.md` D111.

**Files (as actually shipped, superseding the list below):** `src/features/library/{LibraryBrowserPage,LibraryDeckPage,LibraryCollectionView,DeckRow,DeckSettings,FilterMenu,collectionTree,deckMetrics,deckMark}.tsx|ts`, `src/features/library/shared/{CollectionNav,CollectionNavDrawer,DeckMark,MasteryRing,MeterBar,EmptyState,Stat,sortDecks,useIsWideLibrary}.tsx|ts` (adapted from `design-preview/library-shared/*`, not imported from it — design-preview stays untouched/isolated), `src/components/layout/{TopNav,primaryNavLinks,CreateMenu,ProfileMenu}.tsx`.

~~**Depends on:** Phase G (the migration must have run and been verified against real data before the UI that assumes the new ontology ships)~~ **— superseded: shipped without Phase G, against the UI-only derivation above.** Phase F (the "Add Card" action needs Create/Edit built against the new interaction types) — satisfied, all six types were already shippable.
**Acceptance:** ~~every Collection/Deck in the migrated data renders correctly with zero cards lost (verified against Phase G's report)~~ — not applicable, no migration ran; every *derived* Collection/Deck renders correctly with zero cards lost (no data was moved or transformed at all, only read differently); the new global shell replaces the old one for every route — met; Roadmaps' route is explicitly hidden from primary nav, not silently broken — met.

**Phase G remains fully open** (preflight report never run) — see the "App Shell and Visual Foundation Convergence" decision entry's D88 for the exact boundary between what this phase shipped and what a real Phase G migration would still change.
...
**Update (2026-07-27):** the "Today-only top-nav shell" above is gone — `TodayShell.tsx` was deleted as part of the App Shell convergence milestone (`itera-decisions.md` D91), and `TodayPage` now renders through the same shared `AppShell`/`TopNav` every other route uses. This satisfies this phase's original shell dependency below; only the real product-logic acceptance criteria remain unmet.
...
Separately from the phase sequence below, the product owner asked for the *entire* app to visually reskin to Itera now, rather than waiting for Phase H's real IA rebuild (Library/Deck/global-shell) to reach every page. This does **not** replace or reorder Phases F–M — it's a parallel, visual-only pass: `AppShell.tsx` now wraps every existing v1 page in `IteraSurface` (`.itera-scope` + light-only `ForceLightTheme`), so Dashboard/Decks/Deck Detail/Roadmaps/Browse/Card Editor/Drafts/Stats/Settings all render with Itera colors automatically (see `itera-decisions.md` D45-D49), while keeping today's data model, page structure, IA, and behavior completely unchanged. ~~When Phase H eventually ships the real Library/Deck rebuild and new global shell, it replaces this pass's shell/reskin for the routes it covers~~ **— done, 2026-07-27: see Phase H's status block and `itera-decisions.md`'s App Shell convergence entry.** Dark mode is off app-wide until an Itera dark palette is designed (Itera's tokens are light-only per spec §36); the toggle is unrendered, not deleted.
