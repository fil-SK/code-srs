...
**Status (2026-07-26):** Recall and Multiple Choice — the first two of the six interaction types, per this phase's own ordering — are done: `CardTypeChooser` (all six tiles, four still marked "Coming soon"), an editor shell per type (`RecallEditorShell`, `MultipleChoiceEditorShell`) each pairing an editor pane with the actual `ReviewSessionScreen`/interaction component as the live preview (desktop split / mobile tabs), and a compact `CardRowV2` in `DeckDetailPage` whose body opens the matching study preview directly, with an inline Edit button and overflow menu (Duplicate/Move/Suspend/Delete) on the row itself. A separate read-only `CardDetailPage` (`/cards/:id`) was built first, then removed after hands-on use showed it was just an extra click in front of what Study/Edit/overflow already cover one level up — see `itera-decisions.md`. The remaining four interaction editors are unbuilt (chooser-only), per this milestone's explicit scope.
...
**Files:** `src/features/cardsV2/` (new: `CardTypeChooser`, `CardCreatePage`, `RecallEditorShell`, `RecallFields`, `RecallLivePreview`, `MultipleChoiceEditorShell`, `MultipleChoiceFields`, `MultipleChoiceOptionRow`, `MultipleChoiceLivePreview`, `CardEditEntry`, `CardStudyPreviewPage`, `CardRowV2`, `shared/{OverflowMenu,StatusBadge,InteractionTypeBadge,interactionTypeMeta,format}` — the last four promoted out of `design-preview/library-shared` once a real (not preview-only) consumer needed them) + `src/domain/cardsV2/{recallForm,saveRecallCard,multipleChoiceForm,saveMultipleChoiceCard,factory}.ts` + `src/hooks/useCardsV2.ts`. `src/features/cards/CardEditorPage.tsx` itself is untouched — it still owns the other 6 v1 types; `CardEditEntry` is the branch point at `cards/:id/edit`.

**Depends on:** Phase E (editor preview embeds production Review components).
**Acceptance:** creating a Recall or Multiple Choice card round-trips through save/reload correctly; each editor's preview pane is pixel-identical to the real Review rendering of the same content. (The original acceptance criterion — "each of the six types" — is scoped down to Recall + Multiple Choice for this milestone; the other four remain to be built.)
