...
**Status (2026-07-27):** Recall, Multiple Choice, Write Code, Ordering, and Matching — five of the six interaction types, per this phase's own ordering — are done: `CardTypeChooser` (all six tiles, one still marked "Coming soon"), an editor shell per type (`RecallEditorShell`, `MultipleChoiceEditorShell`, `WriteCodeEditorShell`, `OrderingEditorShell`, `MatchingEditorShell`) each pairing an editor pane with the actual `ReviewSessionScreen`/interaction component as the live preview (desktop split / mobile tabs), and a compact `CardRowV2` in `DeckDetailPage` whose body opens the matching study preview directly, with an inline Edit button and overflow menu (Duplicate/Move/Suspend/Delete) on the row itself. A separate read-only `CardDetailPage` (`/cards/:id`) was built first, then removed after hands-on use showed it was just an extra click in front of what Study/Edit/overflow already cover one level up — see `itera-decisions.md`. The remaining interaction editor (Walkthrough) is unbuilt (chooser-only), per this milestone's explicit scope.
