...
**Status (2026-07-26):** Recall — the first of the six interaction types, per this phase's own ordering — is done: `CardTypeChooser` (all six tiles, five marked "Coming soon"), `RecallEditorShell` (editor pane + the actual `ReviewSessionScreen`/Recall component as the live preview, desktop split / mobile tabs), and a compact `CardRowV2` in `DeckDetailPage` whose body opens the Recall study preview directly, with an inline Edit button and overflow menu (Duplicate/Move/Suspend/Delete) on the row itself. A separate read-only `CardDetailPage` (`/cards/:id`) was built first, then removed after hands-on use showed it was just an extra click in front of what Study/Edit/overflow already cover one level up — see `itera-decisions.md`. The other five interaction editors are unbuilt (chooser-only), per this milestone's explicit scope.
**A real architectural decision was made ahead of this phase's original file list:** rather than the v1 `CardEditorPage` being reworked in place to write v1 `Card` objects, `CardV2` became genuinely persisted storage — a new `cardsV2` repository member/Dexie table/Supabase table, with its own embedded `scheduling` (mirroring how v1 `CardBase` embeds it), deliberately independent of Phase D's `CardState` extraction (no reads added there, no existing card's scheduling source changed). Legacy v1 `basic`/`codeReading`/`bugFinding` cards are migrated to a `CardV2Record` (same id, so `ReviewLog` history survives) the moment they're edited and saved through the new editor; the superseded v1 row is deleted. See `itera-decisions.md` for the full reasoning and the open question this leaves.
**Known gap, not yet closed:** `CardV2Record`s are not part of the global due queue (`useDueCards`/`ReviewPage` still only reads v1 `cards`) — "Study this Card" and the editor's live preview are both deliberately non-committing, so a `CardV2Record` can be authored and previewed but not yet actually, schedulingly reviewed. Closing this (merging `CardV2Record`s into the due queue, real grading) is unscoped future work.

**Files:** `src/features/cardsV2/` (new: `CardTypeChooser`, `CardCreatePage`, `RecallEditorShell`, `RecallFields`, `RecallLivePreview`, `CardEditEntry`, `CardStudyPreviewPage`, `CardRowV2`, `shared/{OverflowMenu,StatusBadge,InteractionTypeBadge,interactionTypeMeta,format}` — the last four promoted out of `design-preview/library-shared` once a real (not preview-only) consumer needed them) + `src/domain/cardsV2/{recallForm,saveRecallCard,factory}.ts` + `src/hooks/useCardsV2.ts`. `src/features/cards/CardEditorPage.tsx` itself is untouched — it still owns the other 7 v1 types; `CardEditEntry` is the new branch point at `cards/:id/edit`.

**Depends on:** Phase E (editor preview embeds production Review components).
**Acceptance:** creating a Recall card round-trips through save/reload correctly; the preview pane in the editor is pixel-identical to the real Review rendering of the same content. (The original acceptance criterion — "each of the six types" — is scoped down to Recall for this milestone; the other five remain to be built.)
