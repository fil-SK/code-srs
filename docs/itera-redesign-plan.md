...
**Status (2026-07-26):** Recall, Multiple Choice, and Write Code — the first three of the six interaction types, per this phase's own ordering — are done: `CardTypeChooser` (all six tiles, three still marked "Coming soon"), an editor shell per type (`RecallEditorShell`, `MultipleChoiceEditorShell`, `WriteCodeEditorShell`) each pairing an editor pane with the actual `ReviewSessionScreen`/interaction component as the live preview (desktop split / mobile tabs), and a compact `CardRowV2` in `DeckDetailPage` whose body opens the matching study preview directly, with an inline Edit button and overflow menu (Duplicate/Move/Suspend/Delete) on the row itself. A separate read-only `CardDetailPage` (`/cards/:id`) was built first, then removed after hands-on use showed it was just an extra click in front of what Study/Edit/overflow already cover one level up — see `itera-decisions.md`. The remaining three interaction editors (Ordering, Matching, Walkthrough) are unbuilt (chooser-only), per this milestone's explicit scope.
