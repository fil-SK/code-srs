...
- `apps/mobile` is the Expo SDK 54 / React Native presentation and composition workspace; Today, Notifications, Progress, Profile & Settings, Library's All Decks/Collection/Deck depths and fixture-backed previews for all six interactions now have native presentations. Notifications is a local inbox preview rather than OS push functionality, and Review is still not a live repository-backed session.

The current project checkpoint is: **interaction-specific mobile UI review now covers all six interaction types.** The Review tab opens Walkthrough at `/review`; Write Code remains at `/review/write-code`, Multiple Choice at `/review/multiple-choice`, Matching at `/review/matching`, Ordering at `/review/ordering` and Recall at `/review/recall` inside the same immersive nested stack. Walkthrough exercises exact-input, Multiple Choice and Recall steps; highlights each step's focus lines; locks the first submitted response; separates per-step tip/explanation from global card tip/explanation; and produces one shared aggregate result and one final preview rating. It consumes `initialWalkthroughState`, `gradeWalkthroughStep` and `walkthroughBehavior` from core, writes nothing and returns to Today through the exit control. The screen's structured fixture rendering is not a generic native RichText renderer, so the existing native-renderer security TODO remains open and unchanged. Do not mark Review capability-parity complete.
...
The web RichText/content rendering path has received a focused malicious-content and stored-XSS audit; no stored-XSS execution defect remained. The Walkthrough remote-image/network-beacon defect found by that pass was fixed with the shared `isSafeImageSource` allowlist. Once a native content renderer exists, it must receive its own equivalent malicious-content and security regression pass; the web audit does not prove a future native renderer safe.
...
| **Mobile Multiple Choice preview** (`/review/multiple-choice`, with Matching retained at `/review/matching`, `apps/mobile/src/components/review/`) | Fourth interaction-specific native View: shared immersive header/rating chrome, structured technical prompt, selection-mode-aware radio/checkbox semantics, spacious native options, explicit correct/incorrect/missed feedback, subtle reduced-motion-aware submit feedback and preview rating controls. Shared `multipleChoiceBehavior.isResponseReady` and `gradeMultipleChoice` own readiness and exact-set correctness. | Values live in `apps/mobile/src/fixtures/reviewMultipleChoice.ts`. Selection, submit and rating state are local preview interactions only; no queue, FSRS scheduling, persistence or repository is connected. The fixture exercises multiple selection; the View also preserves single-selection behavior. Physical-device review is pending. |
| **Mobile Write Code preview** (`/review/write-code`, with Multiple Choice retained at `/review/multiple-choice`, `apps/mobile/src/components/review/`) | Fifth interaction-specific native View: shared immersive header/rating chrome, structured prompt and language identity, a keyboard-aware native multiline monospace editor seeded with starter code, explicit correct/incorrect feedback, incorrect-only syntax-colored expected answer, explanation and preview rating controls. Shared `writeCodeBehavior.isResponseReady` and `writeCodeBehavior.autoGrade` own readiness and exact accepted-answer comparison. | Values live in `apps/mobile/src/fixtures/reviewWriteCode.ts`. Editor, submit and rating state are local preview interactions only; no code execution, queue, FSRS scheduling, persistence or repository is connected. The lightweight native editor intentionally does not reproduce CodeMirror syntax highlighting while typing. Physical-device review is pending. |
| **Mobile Walkthrough preview** (`/review`, with Write Code retained at `/review/write-code`, `apps/mobile/src/components/review/`) | Sixth interaction-specific native View: shared immersive header/rating chrome, structured scenario and focused code lines, exact-input/Multiple Choice/Recall step renderers, answered/current/future progress, first-submission locking, backward step inspection, distinct step-level and global tip/explanation panels, aggregate card feedback and one final rating. Shared `gradeWalkthroughStep`, `walkthroughBehavior.isResponseReady` and `walkthroughBehavior.autoGrade` own per-step and card-level semantics. | Values live in `apps/mobile/src/fixtures/reviewWalkthrough.ts`. Step drafts, answers, navigation, finish and rating state are local preview interactions only; no image is used, and no queue, FSRS scheduling, persistence or repository is connected. Physical-device review is pending. |
...
Measured 2026-08-24 after the fixture-backed native Progress implementation. The established web/core counts remain unchanged.
...
Mobile is intentionally checked outside the root TypeScript solution: Expo's generated config is not converted into a composite project. `expo-doctor` passes 18/18 checks, Expo's TypeScript 5.9.3 check and `expo lint` are clean. Progress browser QA at 390×844 found no horizontal document overflow; its five KPI cards, hand-built heat map and retention trend, deck performance and milestones remain readable, and the last section clears the persistent tab bar. Physical-device visual acceptance of the Progress GUI remains pending owner review.
