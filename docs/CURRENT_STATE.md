...
Measured 2026-08-25 after M-DEMO-2 (§25). The established web/core counts remain unchanged; only the mobile suite grew.
...
Mobile is intentionally checked outside the root TypeScript solution: Expo's generated config is not converted into a composite project. `expo-doctor` passes 18/18 checks, Expo's TypeScript 5.9.3 check and `expo lint` are clean. **`npx jest` in `apps/mobile` is 28 suites / 333 tests**, up from 14 / 140: M-DEMO-2 added the native RichText renderer and its content-security regression, the demo scheduling and queue, the six interaction wirings, the session, the registry binding, the Review entry screen and the tab-bar rule. Those assertions cover **mobile wiring only** — core's graders, FSRS and parser are proven once under Vitest and are never re-asserted there (master plan D10). Progress browser QA at 390×844 found no horizontal document overflow; its five KPI cards, hand-built heat map and retention trend, deck performance and milestones remain readable, and the last section clears the persistent tab bar. Physical-device visual acceptance of the Progress GUI remains pending owner review.
...
**M-DEMO-3 - make Today and Progress react to the in-memory Demo Review history.** M-DEMO-2 (§25) produces canonical `ReviewLog` entries and advances real `SchedulingState` on the demo cards, and Today's due count and Library's per-deck due counts already follow that scheduling. What does not follow it yet is everything that needs *history*: streak, retention, the activity map, the retention series, milestones and reviews-this-period are still authored deterministic constants (§24's derivation rule). M-DEMO-3 is the milestone that feeds them from `workspace.reviewLogs` through the existing shared calculations in `packages/core/src/domain/stats/` - never a second calculation. It is not started.

Before it, the owner's physical-device pass on M-DEMO-2 is outstanding: Today and the Review tab both starting a real session, tabs hidden during a session, all six interaction types completing, keyboard clearance on Write Code, touch reordering on Ordering, touch matching on Matching, multi-step Walkthrough, objective feedback, an overridable rating recommendation, progress, completion, Undo, exit, and re-entry reflecting the in-memory state. The M-DEMO-1 pass is confirmed complete.
...
- **Reactive Today/Progress.** Due counts follow real scheduling because they always derived from the card list, but streak, retention, the activity map, the retention series and milestones remain authored constants. That is M-DEMO-3.
