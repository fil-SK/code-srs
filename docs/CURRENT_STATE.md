...
- `AccountMenu` + `AccountMenuContent` (`src/components/layout/`). Built on the existing `FloatingPanel` — **no popover dependency was added.** 300px anchored panel with `manageFocus` (focus enters the menu, arrows/Home/End walk it, Tab closes, Escape returns focus to the trigger). Below 480px (`useIsNarrowShell`) the identical content renders as a bottom sheet.
- Live rows: **Account settings** and **Sign out** (enabled whenever any session exists — local, demo, or Supabase).
- Placeholder rows: Preferences, Keyboard shortcuts, Help & documentation, About Itera — focusable `aria-disabled` rows with a "Soon" pill (never `disabled`, never hidden).
- **It is quick navigation only.** Do not grow it into a second settings sidebar; new settings go in `src/features/settings/`.
...
- **`/preview`'s page chrome is still v1-styled** while the card inside it is the real v2 experience.
