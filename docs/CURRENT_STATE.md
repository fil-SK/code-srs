...
- **The session ends in a real summary (D454).** `SessionCompleteCard.tsx` reports cards reviewed, recall, time spent answering, the Again/Hard/Good/Easy split, the current streak (with "extended today" only when this session moved it), cards seen for the first time, and the soonest next-due. Every value comes from `summarizeSession` (`packages/core/src/domain/review/sessionSummary.ts`) over the ReviewLogs the session itself wrote - no new entity, no persisted achievement and no scoring model beside FSRS. A session with no mature card shows an em dash for recall rather than a fabricated 100%. `Undo last` and `Back to Today` are unchanged, and the card enters on the 420ms `.itera-complete-in` completion transition.
- **The reveal settles rather than snapping (D456).** All six types already flipped; what jumped was the card's own height (measured at -80px for Multiple Choice, +140px for Ordering) in the same frame the explanation and rating controls appeared beneath it. The panels now arrive together on `.reveal-in` and `ReviewSessionScreen` animates the card height across the transition. Reduced motion disables both without gating any content.
- **Entering a session no longer paints a loading word.** `/review`'s pending branch renders the session strip plus a card-shaped placeholder in the real card's geometry (D455).
...
app/(app)/deck/[deckId]/index.tsx  deck detail, above the tabs (§33, D439)
...
Group segments do not appear in a URL, so **every existing path is unchanged** (`/today`, `/library`, `/review`, `/progress`, `/profile`, `/notifications`; deck detail moved from `/library/deck/[deckId]` to `/deck/[deckId]` in §33) and no existing `router.push` was touched. The root layout is now a `Stack` rather than a `Slot`, which is what the Protected API requires and which also gives `/notifications` a real push transition and the iOS back-swipe it previously lacked.
...
- `/deck/[deckId]` (then `/library/deck/[deckId]`, moved in §33) and `/library/[collectionId]` resolve their parameter. Both previously ignored it: every deck link showed Modern C++, and every collection link that was not Languages & C++ showed Interview Core. An unknown id now gets a not-found state.
...
## 33. Four defects found in owner testing (2026-08-27)

Reported after using the demo build; each was a real defect rather than a preference, and all four are fixed. Verified in a browser against the Expo web build at 390x844 and, for the web heat map, against the running web app at 1440x900 with seeded review history. **Physical-device verification remains the owner's and is not claimed.**

| Reported | Cause | Fix |
|---|---|---|
| Back from a deck opened on Today returned to Library / All Decks | The deck was a route *inside* the Library tab's stack, so opening it from Today switched tabs and Back popped that stack | The deck moved to `app/(app)/deck/[deckId]/index.tsx`, above the tabs (D439) |
| The deck screen's identity mark is a rectangle | It was 84x102 while every other deck and collection mark is square | 84x84 (D442) |
| A Collection's *cards* metric used the Library icon | `book-open-page-variant-outline` is the Library tab's own glyph | `cards-outline`, the same glyph the deck screen's cards metric uses (D442) |
| The heat map is unreadable and has no range control | It was a wrapping run of 24px squares - a row was neither a week nor a month, no cell said which day it was, and the 30D/3M/1Y group had been removed by D411 | A week-column calendar grid with weekday rows, month labels and a working range group (D440, D441) |

### Deck detail is now a route above the tabs

`/library/deck/[deckId]` became `/deck/[deckId]`, joining `card/[cardId]/study` and the four authoring routes at the `(app)` level. Back now returns to whichever surface pushed the deck - Today, Progress, All Decks or a Collection - and the Library tab is no longer left displaying a deck it was never asked to open. Every push site moved with it (Today, Progress, Notifications, All Decks, Collection, and the three `replace` exits from the authoring routes), as did the collection redirect. **Consequence, accepted deliberately:** a deck detail is a pushed detail surface now, so the tab bar is not drawn behind it - already true of Card study and every authoring screen.

Mobile's typed-route check depends on `.expo/types/router.d.ts`, which only the dev server regenerates. `npx expo start` produces the correct union for this tree; a stale or web-mode-generated file types the index route as `/deck/[deckId]/index` and fails `npx tsc --noEmit` at eight call sites. Delete it and let `npx expo start` rewrite it before believing that failure.

### Progress reports a chosen window again

The route holds one `DateRangePreset` and passes it to the one `demoProgressViewModel` call, so the KPI block, its period-over-period deltas, the activity grid, the retention chart and the date pill can never disagree about the window. The group is **30D / 3M / 1Y**, matching the mobile Progress mockup; 7D is not offered. The KPI heading is the preset's own label ("Last 30 days") rather than a hardcoded "This month", and the retention series caps at 30 buckets so a year is not asked for 365 of them. This supersedes D411, whose objection was to the layout, not the ranges.

### The activity heat map is a calendar

One column per week, one row per weekday, months named above the column they begin in, and the existing Less/More legend. Cells grow to fill a short range and shrink for a long one down to a 10-point floor; past that the grid scrolls sideways inside the card and **opens on the most recent end**, so a year shows the visitor's own recent days rather than twelve empty months. A range narrow enough to fit is centred. Every cell is now labelled with the day it stands for ("Today, 3 reviews") instead of a bare count.

The week bucketing and month labelling moved into `packages/core/src/domain/stats/progressMetrics.ts` as `toHeatmapWeeks` (generic over `T extends HeatmapDay`, so a platform may decorate a day) and `heatmapMonthLabels`; web's `ActivityHeatmap.tsx` calls them instead of its own copies and renders identically. `DATE_RANGE_PRESETS` gained a `short` label for a phone-width control (D441).

### Gates

```
apps/mobile: npx jest              -> 39 suites, 547 tests, passing
apps/mobile: npx tsc --noEmit      -> clean (after a fresh `npx expo start` typegen)
root:        npx vitest run        -> 113 files, 1088 tests, passing
root:        npx tsc -b --force    -> clean
root:        npm run lint          -> clean
```
