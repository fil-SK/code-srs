...
|---|---|---|
| `Button.tsx` | Base button | `variant?: 'primary' \| 'secondary' \| 'ghost'` (default `secondary`). `primary`: `bg-accent text-white hover:brightness-110`. `secondary`: `border border-border bg-panel-2 hover:border-accent`. `ghost`: `text-muted hover:bg-panel`. Base radius `rounded-[9px]`, disabled → `opacity-50 pointer-events-none`. |
