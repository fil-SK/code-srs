...
| Editor + preview | 1440px | `CardEditorShell`'s own animated `max-width`, growing in step with the preview drawer. Its action header (Preview / Back to deck / Save) is sticky, parking below the create page's equally-sticky interaction chooser via a measured offset — see D167 |
